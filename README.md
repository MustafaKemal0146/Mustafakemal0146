<div align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=24&duration=3000&pause=1000&color=00D9FF&center=true&vCenter=true&width=1000&lines=Merhaba%2C+Ben+Mustafa+Kemal+%C3%87%C4%B1ng%C4%B1l+%F0%9F%91%8B;AI+%26+Cybersecurity+Developer+%F0%9F%A4%96;Music+Producer+%26+Code+Creator+%F0%9F%8E%B5;Bilgiyi+Payla%C5%9F%C4%B1yor%2C+Gelece%C4%9Fi+%C5%9Eekillendiriyor+%F0%9F%9A%80" alt="Typing SVG" />
</div>

<div align="center">
  <img src="https://user-images.githubusercontent.com/74038190/225813708-98b745f2-7d22-48cf-9150-083f1b00d6c9.gif" width="500">
</div>

<h2 align="center">🚀 Yapay Zeka, Siber Güvenlik ve Teknoloji Tutkunu</h2>

---

## 🔗 Benimle Bağlantı Kur

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/mustafakemal0146)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/MustafaKemal0146)
[![Instagram](https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/mustafakemal0146)
[![YouTube](https://img.shields.io/badge/YouTube-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://www.youtube.com/@mustafakemalcingil)
[![Website](https://img.shields.io/badge/Website-1A1A1A?style=for-the-badge&logo=About.me&logoColor=white)](https://mustafakemalcingil.site/)

</div>

---

## 🛠️ Teknoloji Yığını

<div align="center">

### 💻 Programlama Dilleri
<img src="https://skillicons.dev/icons?i=python,javascript,typescript,java,c" />

### 🎨 Frontend
<img src="https://skillicons.dev/icons?i=react,tailwind,html,css,nextjs" />

### ⚙️ Backend & Veritabanı
<img src="https://skillicons.dev/icons?i=nodejs,supabase,postgresql,firebase,mongodb" />

### 🤖 AI/ML
<img src="https://skillicons.dev/icons?i=opencv,tensorflow,git,linux,docker" />

### 🛡️ Siber Güvenlik
<img src="https://skillicons.dev/icons?i=kali,bash,vim,vscode,figma" />

</div>

---

## 🚀 Öne Çıkan Projeler

<table>
<tr>
<td width="50%">

### ⚔️ SETH
> Terminalde çalışan **otonom AI ajan** — siber güvenlik, CTF çözümü ve kod analizi için. 12 AI sağlayıcı, 60+ araç, Türkçe arayüz, kalıcı bellek, multi-agent koordinatör.
>
> `TypeScript` `AI Agent` `Cybersecurity` `CTF` `CLI`
>
> [![Repo](https://img.shields.io/badge/GitHub-View_Code-black?style=for-the-badge&logo=github)](https://github.com/MustafaKemal0146/seth) [![Website](https://img.shields.io/badge/Website-Visit-blue?style=for-the-badge&logo=About.me)](https://seth.mustafakemalcingil.site)

</td>
<td width="50%">

### 🎼 Codchestra
> AI iş akışı orkestratörü — `codchestra.tasks.md` dosyasındaki görevleri alır, **Codex/ChatGPT'yi bir döngüde çalıştırır** ve tüm görevler tamamlanana kadar durdurmaz.
>
> `TypeScript` `AI Orchestration` `CLI` `Loop Engine`
>
> [![Repo](https://img.shields.io/badge/GitHub-View_Code-black?style=for-the-badge&logo=github)](https://github.com/MustafaKemal0146/codchestra)

</td>
</tr>
<tr>
<td width="50%">

### 🐝 C.U.T.E
> **Council-based Unified Thinking Engine** — Birden fazla AI ajanını bir konseyde tartıştıran karar destek sistemi. 4 fazlı Swarm Protokolü, 30 konsey şablonu, 3D görselleştirme.
>
> `TypeScript` `React` `Python` `Multi-Agent` `Docker`
>
> [![Repo](https://img.shields.io/badge/GitHub-View_Code-black?style=for-the-badge&logo=github)](https://github.com/MustafaKemal0146/CUTE)

</td>
<td width="50%">

### 🤖 MCP GitHub README Generator
> GitHub projelerini analiz ederek otomatik profesyonel README dosyası oluşturan AI destekli araç.
>
> `AI` `GitHub` `Automation` `Multi-LLM`
>
> [![Repo](https://img.shields.io/badge/GitHub-View_Code-black?style=for-the-badge&logo=github)](https://github.com/MustafaKemal0146/github-readme-generator-mcp)

</td>
</tr>
<tr>
<td width="50%">

### 🎵 TenFastCoderFinger
> Programcılar için özel tasarlanmış yazma hızı pratik uygulaması.
>
> `React` `TypeScript` `Gaming` `Education`
>
> [![Repo](https://img.shields.io/badge/GitHub-View_Code-black?style=for-the-badge&logo=github)](https://github.com/MustafaKemal0146/10fastcoderfinger)

</td>
<td width="50%">

### 🎶 AI Müzik Kontrolü
> Kafa hareketleriyle sistem genelinde medya kontrolü — OpenCV + MediaPipe.
>
> `Python` `OpenCV` `AI` `MediaPipe`
>
> [![Repo](https://img.shields.io/badge/GitHub-View_Code-black?style=for-the-badge&logo=github)](https://github.com/MustafaKemal0146/head-movment-music-control)

</td>
</tr>
</table>

---

## 🔌 MCP Sunucularım

<table>
<tr>
<td width="33%">

### ⚡ turkey-power-mcp
> Türkiye'deki **tüm 20 elektrik dağıtım şirketini** tek MCP'de toplar. Şehir/ilçe yaz, kesinti ve arıza bilgisi gerçek zamanlı gelsin.
>
> `Python` `MCP` `81 İl` `20 EDAŞ`
>
> [![Repo](https://img.shields.io/badge/GitHub-View_Code-black?style=for-the-badge&logo=github)](https://github.com/MustafaKemal0146/turkey-power-mcp)

</td>
<td width="33%">

### 🌍 kandilli-mcp
> Kandilli Rasathanesi (KOERI) deprem verilerini Claude ve AI asistanlara bağlayan MCP sunucusu. 9 araç, gerçek zamanlı veri.
>
> `Python` `MCP` `Earthquake` `KOERI`
>
> [![Repo](https://img.shields.io/badge/GitHub-View_Code-black?style=for-the-badge&logo=github)](https://github.com/MustafaKemal0146/kandilli-mcp)

</td>
<td width="33%">

### 🎓 trdizin-mcp
> TÜBİTAK **TR Dizin** akademik veritabanına MCP üzerinden erişim. API key gerektirmez, yayın/yazar/dergi/kurum arama.
>
> `JavaScript` `MCP` `Academic` `TÜBİTAK`
>
> [![Repo](https://img.shields.io/badge/GitHub-View_Code-black?style=for-the-badge&logo=github)](https://github.com/MustafaKemal0146/trdizin-mcp)

</td>
</tr>
</table>

---

## 📊 GitHub Analitiği

<div align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=MustafaKemal0146&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true"/>
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=MustafaKemal0146&layout=compact&langs_count=8&theme=tokyonight"/>
</div>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=MustafaKemal0146&theme=tokyonight" alt="GitHub Streak" />
</div>

<div align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=MustafaKemal0146&theme=tokyonight&no-frame=true&no-bg=false&margin-w=4&row=1" />
</div>

---

## 📈 Kodlama Aktivitesi

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=MustafaKemal0146&bg_color=1a1b27&color=70a5fd&line=70a5fd&point=bf91f3&area=true&hide_border=true" />
</div>

---

## 🤓 Hakkımda

<div align="center">

```javascript
const mustafaKemal = {
    pronouns: "he/him",
    code: ["Python", "JavaScript", "TypeScript", "Java"],
    askMeAbout: ["AI Agents", "MCP", "Cybersecurity", "Music Production"],
    technologies: {
        aiAgents:  ["SETH", "C.U.T.E", "Codchestra"],
        mcp:       ["turkey-power-mcp", "kandilli-mcp", "trdizin-mcp"],
        frontEnd:  ["React", "Next.js", "Tailwind"],
        backEnd:   ["Node", "FastAPI", "Express"],
        databases: ["PostgreSQL", "MongoDB", "Supabase"],
        misc:      ["Docker", "Linux", "OpenCV", "MediaPipe"]
    },
    currentFocus: "Building autonomous AI agents & MCP servers",
    funFact:      "I can debug code and produce music at the same time! 🎵🐛"
};
```

</div>

---

## 🐍 Katkı Yılanı

<div align="center">
  <img alt="snake eating my contributions" src="https://raw.githubusercontent.com/MustafaKemal0146/MustafaKemal0146/output/github-contribution-grid-snake-dark.svg" />
</div>

---

<div align="center">

### *"Kod yazmak sanat, AI ile çalışmak ise geleceği şekillendirmektir"*

<img src="https://komarev.com/ghpvc/?username=MustafaKemal0146&color=blueviolet&style=for-the-badge&label=Profile+Views" />

**Made with ❤️ by Mustafa Kemal Çıngıl**

</div>
